...
export const Payment = (props: PaymentProps) => {
  ...
  const handleSubmit = async (values: PaymentRecord & { name: string }) => {
    const wrapped = wrapperBhai(
      addPayment,
      true,
      "Successfully added your payment"
    );
    const data = await wrapped(user?.uid, {
      ...values,
      user_phone_number: user.phoneNumber,
      phoneNumber: formatPhoneNumber(values.phoneNumber.toString()) ?? "",
    });
    data &&
      navigate(`/success`, {
        state: {
          ...
          description:
            "We will get back to you in 2 business days, please email us at kongtsey@gmail.com if you have any inquiries.",
        },
      });
  };
  return (
        ...
        {(formik) => {
          return (
            <Form>
              ...
              >
                <Box w={"full"} maxW={"md"} m={2}>
                  <StatsCard
                    title="Subscription Amount"
                    stat="Nu 1000 | 1500 | 6500"
